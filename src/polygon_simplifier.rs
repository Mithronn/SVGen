...
mod quadric {
    ...
    fn to_tensor_matrix_inverse(q: &Quadric, epsilon: f64) -> Option<[f64; 3]> {
        let det: f64 = (q.a2 * q.b2) - (q.ab * q.ab);
        if det.abs() > epsilon {
    ...
    pub fn add(q_a: &Quadric, q_b: &Quadric) -> Quadric {
        return Quadric {
            a2: q_a.a2 + q_b.a2,
            ab: q_a.ab + q_b.ab,
            ac: q_a.ac + q_b.ac,
    ...
    pub fn iadd(q_a: &mut Quadric, q_b: &Quadric) {
        q_a.a2 += q_b.a2;
        q_a.ab += q_b.ab;
        q_a.ac += q_b.ac;
        q_a.b2 += q_b.b2;
    ...
    pub fn from_plane(v: &[f64; 3]) -> Quadric {
        return Quadric {
            a2: v[0] * v[0],
            b2: v[1] * v[1],
    ...
    pub fn evaluate(q: &Quadric, v: &DVec2) -> f64 {
        return (q.a2 * v.x * v.x)
            + (q.ab * 2.0 * v.x * v.y)
            + (q.ac * 2.0 * v.x)
            + (q.b2 * v.y * v.y)
    ...
    pub fn optimize(q: &Quadric, epsilon: f64) -> Option<DVec2> {
        if let Some(m) = to_tensor_matrix_inverse(q, epsilon) {
            // 3x3 matrix multiply & negate
            // (ac, bc) == (x, y).
            return Some(DVec2::new(-(m[0] * q.ac), -(m[1] * q.ac + m[2] * q.bc)));
        } else {
            return None;
        }
    }
...
#[inline(always)]
fn plane_from_point_normal(p: &DVec2, n: &DVec2) -> [f64; 3] {
    [n.x, n.y, -p.dot(*n)]
}
...
fn edge_heap_insert(
    ...
    let optimize_co = {
        if let Some(optimize_co) = quadric::optimize(&quadric::add(q1, q2), f64::EPSILON) {
            ...
            DVec2::new((v1.x + v2.x) / 2.0, (v1.y + v2.y) / 2.0)
        }
    };

    let cost = (quadric::evaluate(q1, &optimize_co) + quadric::evaluate(q2, &optimize_co)).abs();
...
const INVALID_CO: DVec2 = DVec2::splat(::std::f64::MAX);

fn edge_heap_collapse(
    poly_edit: &mut Vec<DVec2>,
    ...
    for i_other in &[
        i_prev,
        edges[i_prev].index_prev,
        i_next,
        edges[i_next].index_next,
    ] {
...
pub fn poly_simplify(is_cyclic: bool, poly: &Vec<DVec2>, simplify_threshold: f64) -> Vec<DVec2> {
    ...
    for e in &mut edges {
        ...
        let mut point = DVec2::new(-(p1.y - p2.y), p1.x - p2.x);
        let len = point.normalize();
        if len != 0.0 {
            let p = plane_from_point_normal(p1, &point);
...
pub fn poly_list_simplify(poly_list_src: &mut Vec<(bool, Vec<DVec2>)>, simplify_threshold: f64) {
